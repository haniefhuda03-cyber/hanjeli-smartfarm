...
import { BrainCircuit, Sparkles, AlertCircle, TrendingUp, Bug, Droplet } from "lucide-react"
    ...
    <div className="relative min-h-screen w-full bg-page px-5 md:px-6 lg:px-8 pt-16 lg:pt-6 pb-28 lg:pb-8">
      ...
      <div className="mb-8">
        <h1 className="text-2xl md:text-3xl font-bold text-foreground flex items-center gap-2">
          ...
          {t('nav.ai_analysis', 'Analisis AI')}
        </h1>
        <p className="text-sm md:text-base text-foreground/60 mt-1.5 max-w-2xl">
          {t('ai.analysisSubtitle', 'Dapatkan wawasan cerdas, prediksi prediktif, dan rekomendasi otomatis untuk mengoptimalkan pertumbuhan kebun Hanjeli Anda menggunakan kecerdasan buatan.')}
        </p>
      </div>
      ...
      <div className="bg-surface-sage rounded-4xl p-8 lg:p-16 border border-white/70 neu-raised flex flex-col items-center justify-center text-center relative overflow-hidden">
        ...
        <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none opacity-20">
          ...
          <div className="absolute bottom-[-10%] right-[-5%] w-64 h-64 bg-amber-500/20 rounded-full blur-3xl"></div>
        </div>
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-3 font-display">
            {t('ai.analysisEmptyTitle', 'Sistem AI Sedang Mempelajari Lahan')}
          </h2>

          <p className="text-sm md:text-base text-foreground/70 max-w-xl mx-auto mb-8 leading-relaxed">
            {t('ai.analysisEmptyDesc', 'Modul Analisis AI sedang dilatih menggunakan data historis agronomi Hanjeli. Fitur ini akan segera hadir untuk memberikan Anda rekomendasi yang presisi berbasis data.')}
          </p>
      ...
      <div className="mt-8">
        <h3 className="text-lg font-bold text-foreground mb-4 pl-2 border-l-4 border-primary">{t('ai.analysisUpcomingHeading', 'Apa yang akan datang?')}</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {[
            {
              title: t('ai.analysisFeature1Title', 'Prediksi Panen'),
              desc: t('ai.analysisFeature1Desc', 'Estimasi waktu dan jumlah hasil panen Hanjeli berdasarkan tren kelembapan, cuaca, dan NPK.'),
              icon: TrendingUp,
              color: "text-blue-600",
              bg: "bg-blue-100"
            },
            {
              title: t('ai.analysisFeature2Title', 'Deteksi Dini Hama'),
              desc: t('ai.analysisFeature2Desc', 'Analisis anomali pada sensor dan cuaca untuk memberikan peringatan dini terhadap potensi serangan hama.'),
              icon: Bug,
              color: "text-amber-600",
              bg: "bg-amber-100"
            },
            {
              title: t('ai.analysisFeature3Title', 'Rekomendasi Pemupukan'),
              desc: t('ai.analysisFeature3Desc', 'Saran takaran nutrisi Nitrogen, Fosfor, dan Kalium (NPK) yang optimal disesuaikan dengan fase tanam.'),
              icon: Droplet,
